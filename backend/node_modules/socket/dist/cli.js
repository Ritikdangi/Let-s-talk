...
async function npmFix(
  ...
  const alertsMap = purls.length
    ...
    : await shadowInject.getAlertsMapFromArborist(
        arb,
        getAlertMapOptions({
          limit
        })
    ...
    for (const pkgJsonPath of pkgJsonPaths) {
      ...
      for (const oldVersion of oldVersions) {
        ...
        for (const {
          ...
          const newVersion = shadowInject.findBestPatchVersion(
            node,
            availableVersions,
            vulnerableVersionRange
          )
...
const { NPM: NPM$8, PNPM: PNPM$6 } = constants
...
const { NPM: NPM$7 } = registryConstants
...
const { DRY_RUN_BAILING_NOW: DRY_RUN_BAILING_NOW$p } = constants
const config$r = {
  commandName: 'npm',
...
const { DRY_RUN_BAILING_NOW: DRY_RUN_BAILING_NOW$o } = constants
const config$q = {
  commandName: 'npx',
...
const { SOCKET_WEBSITE_URL: SOCKET_WEBSITE_URL$2 } = constants
